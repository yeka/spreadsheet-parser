...
class OLERead
{
    ...
    {
        static::$IDENTIFIER_OLE = pack("CCCCCCCC", 0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1);
        ...
        if (substr($this->data, 0, 8) != static::$IDENTIFIER_OLE) {
            $this->error = 1;

            return false;
        }
        $this->numBigBlockDepotBlocks = static::GetInt4d($this->data, static::NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
        $this->sbdStartBlock = static::GetInt4d($this->data, static::SMALL_BLOCK_DEPOT_BLOCK_POS);
        $this->rootStartBlock = static::GetInt4d($this->data, static::ROOT_START_BLOCK_POS);
        $this->extensionBlock = static::GetInt4d($this->data, static::EXTENSION_BLOCK_POS);
        $this->numExtensionBlocks = static::GetInt4d($this->data, static::NUM_EXTENSION_BLOCK_POS);

        $bigBlockDepotBlocks = array();
        $pos = static::BIG_BLOCK_DEPOT_BLOCKS_POS;
        $bbdBlocks = $this->numBigBlockDepotBlocks;
        if ($this->numExtensionBlocks != 0) {
            $bbdBlocks = (static::BIG_BLOCK_SIZE - static::BIG_BLOCK_DEPOT_BLOCKS_POS) / 4;
        }

        for ($i = 0; $i < $bbdBlocks; $i++) {
            $bigBlockDepotBlocks[$i] = static::GetInt4d($this->data, $pos);
            $pos += 4;
        }


        for ($j = 0; $j < $this->numExtensionBlocks; $j++) {
            $pos = ($this->extensionBlock + 1) * static::BIG_BLOCK_SIZE;
            $blocksToRead = min($this->numBigBlockDepotBlocks - $bbdBlocks, static::BIG_BLOCK_SIZE / 4 - 1);
            ...
            if ($bbdBlocks < $this->numBigBlockDepotBlocks) {
                $this->extensionBlock = static::GetInt4d($this->data, $pos);
            }
        }
        ...
        for ($i = 0; $i < $this->numBigBlockDepotBlocks; $i++) {
            $pos = ($bigBlockDepotBlocks[$i] + 1) * static::BIG_BLOCK_SIZE;
            //echo "pos = $pos";
            for ($j = 0; $j < static::BIG_BLOCK_SIZE / 4; $j++) {
                $this->bigBlockChain[$index] = static::GetInt4d($this->data, $pos);
                $pos += 4;
                $index++;
            }
        }
        ...
        while ($sbdBlock != -2) {
            $pos = ($sbdBlock + 1) * static::BIG_BLOCK_SIZE;
            for ($j = 0; $j < static::BIG_BLOCK_SIZE / 4; $j++) {
                $this->smallBlockChain[$index] = static::GetInt4d($this->data, $pos);
                $pos += 4;
                $index++;
            }
    ...
    function __readData($bl)
    {
        ...
        $data = '';
        while ($block != -2) {
            $pos = ($block + 1) * static::BIG_BLOCK_SIZE;
            $data = $data . substr($this->data, $pos, static::BIG_BLOCK_SIZE);
    ...
    function __readPropertySets()
    {
        ...
        while ($offset < strlen($this->entry)) {
            $d = substr($this->entry, $offset, static::PROPERTY_STORAGE_BLOCK_SIZE);
            $nameSize = ord($d[static::SIZE_OF_NAME_POS]) | (ord($d[static::SIZE_OF_NAME_POS + 1]) << 8);
            $type = ord($d[static::TYPE_POS]);
            $startBlock = static::GetInt4d($d, static::START_BLOCK_POS);
            $size = static::GetInt4d($d, static::SIZE_POS);
            $name = '';
            ...
            $offset += static::PROPERTY_STORAGE_BLOCK_SIZE;
        }
    }

    function getWorkBook()
    {
        if ($this->props[$this->wrkbook]['size'] < static::SMALL_BLOCK_THRESHOLD) {
            $rootdata = $this->__readData($this->props[$this->rootentry]['startBlock']);
            $streamData = '';
            $block = $this->props[$this->wrkbook]['startBlock'];
            $pos = 0;
            while ($block != -2) {
                $pos = $block * static::SMALL_BLOCK_SIZE;
                $streamData .= substr($rootdata, $pos, static::SMALL_BLOCK_SIZE);
                $block = $this->smallBlockChain[$block];
            }

            return $streamData;
        } else {
            $numBlocks = $this->props[$this->wrkbook]['size'] / static::BIG_BLOCK_SIZE;
            if ($this->props[$this->wrkbook]['size'] % static::BIG_BLOCK_SIZE != 0) {
                $numBlocks++;
            }
            ...
            while ($block != -2) {
                $pos = ($block + 1) * static::BIG_BLOCK_SIZE;
                $streamData .= substr($this->data, $pos, static::BIG_BLOCK_SIZE);
                $block = $this->bigBlockChain[$block];
            }
...
class Spreadsheet_Excel_Reader
{
    ...
    {
        $this->_defaultFormat = static::SPREADSHEET_EXCEL_READER_DEF_NUM_FORMAT;
    ...
    function _parse()
    {
        ...
        $code = static::v($data, $pos);
        $length = static::v($data, $pos + 2);
        $version = static::v($data, $pos + 4);
        $substreamType = static::v($data, $pos + 6);

        $this->version = $version;

        if (($version != static::SPREADSHEET_EXCEL_READER_BIFF8) &&
            ($version != static::SPREADSHEET_EXCEL_READER_BIFF7)
        ) {
            return false;
        }

        if ($substreamType != static::SPREADSHEET_EXCEL_READER_WORKBOOKGLOBALS) {
            return false;
        }
        ...
        while ($code != static::SPREADSHEET_EXCEL_READER_TYPE_EOF) {
            switch ($code) {
                case static::SPREADSHEET_EXCEL_READER_TYPE_SST:
                    $spos = $pos + 4;
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = static::v($data, $spos);
                            $conlength = static::v($data, $spos + 2);
                            if ($opcode != 0x3c) {
                                return -1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = static::v($data, $spos);
                            $spos += 2;
                        }
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_FILEPASS:
                    return false;
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_NAME:
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    $indexCode = static::v($data, $pos + 4);
                    if ($version == static::SPREADSHEET_EXCEL_READER_BIFF8) {
                        $numchars = static::v($data, $pos + 6);
                        if (ord($data[$pos + 8]) == 0) {
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_FONT:
                    $height = static::v($data, $pos + 4);
                    $option = static::v($data, $pos + 6);
                    $color = static::v($data, $pos + 8);
                    $weight = static::v($data, $pos + 10);
                ...
                case static::SPREADSHEET_EXCEL_READER_TYPE_PALETTE:
                ...
                case static::SPREADSHEET_EXCEL_READER_TYPE_XF:
                    ...
                    $this->xfRecords[] = $xf;
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_NINETEENFOUR:
                    $this->nineteenFour = (ord($data[$pos + 4]) == 1);
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                    ...
                    } elseif ($version == static::SPREADSHEET_EXCEL_READER_BIFF7) {
    ...
    function _parsesheet($spos)
    {
        ...
        if (($version != static::SPREADSHEET_EXCEL_READER_BIFF8) && ($version != static::SPREADSHEET_EXCEL_READER_BIFF7)) {
            return -1;
        }

        if ($substreamType != static::SPREADSHEET_EXCEL_READER_WORKSHEET) {
            return -2;
        }
        $spos += $length + 4;
        $previousRow = null;
        ...
        while ($cont) {
            ...
            if ($lowcode == static::SPREADSHEET_EXCEL_READER_TYPE_EOF) break;
            ...
            unset($this->rectype);
            switch ($code) {
                case static::SPREADSHEET_EXCEL_READER_TYPE_DIMENSION:
                    if (!isset($this->numRows)) {
                        if (($length == 10) || ($version == static::SPREADSHEET_EXCEL_READER_BIFF7)) {
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_MERGEDCELLS:
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_RK:
                case static::SPREADSHEET_EXCEL_READER_TYPE_RK2:
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_MULRK:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                ...
                case static::SPREADSHEET_EXCEL_READER_TYPE_FORMULA:
                case static::SPREADSHEET_EXCEL_READER_TYPE_FORMULA2:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_BOOLERR:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $string = ord($data[$spos + 6]);
                    $this->addcell($row, $column, $string);
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_STRING:
                    // http://code.google.com/p/php-excel-reader/issues/detail?id=4
                    if ($version == static::SPREADSHEET_EXCEL_READER_BIFF8) {
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_ROW:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_DBCELL:
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_MULBLANK:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $cols = ($length / 2) - 3;
                    for ($c = 0; $c < $cols; $c++) {
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_LABEL:
                    $row = ord($data[$spos]) | ord($data[$spos + 1]) << 8;
                    $column = ord($data[$spos + 2]) | ord($data[$spos + 3]) << 8;
                    $this->addcell($row, $column, substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7]) << 8));
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_EOF:
                    $cont = false;
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_HYPER:
                    ...
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_DEFCOLWIDTH:
                    $this->defaultColWidth = ord($data[$spos + 4]) | ord($data[$spos + 5]) << 8;
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_STANDARDWIDTH:
                    $this->standardColWidth = ord($data[$spos + 4]) | ord($data[$spos + 5]) << 8;
                    break;
                case static::SPREADSHEET_EXCEL_READER_TYPE_COLINFO:
    ...
    function _getCellDetails($spos, $numValue, $column)
    {
        ...
        if ($type == 'date') {
            ...
            $utcDays = floor($numValue - ($this->nineteenFour ? static::SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : static::SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS));
            $utcValue = ($utcDays) * static::SPREADSHEET_EXCEL_READER_MSINADAY;
            $dateinfo = static::gmgetdate($utcValue);

            $raw = $numValue;
            $fractionalDay = $numValue - floor($numValue) + .0000001; // The .0000001 is to fix for php/excel fractional diffs

            $totalseconds = floor(static::SPREADSHEET_EXCEL_READER_MSINADAY * $fractionalDay);
    ...
    static function gmgetdate($ts = null)
    {
        ...
        return (static::array_comb($k, explode(":", gmdate('s:i:G:j:w:n:Y:z:l:F:U', is_null($ts) ? time() : $ts))));
    }
